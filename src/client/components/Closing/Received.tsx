...
export const Received: NextPage<Props> = function ({ receivedItemsList, callback }) {
    ...
    <React.Fragment>
        ...
        <table className="table card-table table-hover table-outline">
          ...
          <tbody>
            ...
            {receivedItems && receivedItems?.length !== 0 && (
              <React.Fragment>
                {receivedItems?.map((received: ReceivedItemsInput, i: number) => {
                  return (
                    <tr key={i}>
      ...
      <form
        onSubmit={(e) => {
          ...
          setReceivedItems((currentState) => [...currentState, newReceivedItem]);
